...
これらのインストールスクリプトの使い方については、[「一括インストール・スクリプト」]({{ site.baseurl }}/ja/doc/appendix/bulk_installation_script)のページをご覧ください。
...
これらの必要なツールをインストールする際、Ubuntuなどでは上で紹介した[「一括インストール・スクリプト」]({{ site.baseurl }}/ja/doc/appendix/bulk_installation_script)を利用することもできます。
