...
## Overview
Activates RT Components in the Inactive state.

## Options

```
 -e EC_ID, --exec_context=EC_ID
 　　　　　　ID of the Execution context whose state you want to change. The default is 0.
...
## Examples
- Activates ConsoleOut0.rtc on all owned ECs and participanting ECs.

```
 $ rtact /localhost/local.host_cxt/ConsoleOut0.rtc
```

- Activates ConsoleOut0.rtc and ConsoleIn0.rtc on all owned ECs and participanting ECs.

```
 $ rtact ConsoleOut0.rtc ConsoleIn0.rtc
```

- Activates Motor0.rtc only on the EC whose ID is 2.
