...
## Overview
Disconnects connections between ports. You can also disconnect all connections to a single port, or all connections of a component.

## Options (OPTION)
```
 -i ID, --id=ID Connection (connector) ID.
...
## Examples
- Disconnects all connections between the out port of ConsoleIn0.rtc and the in port of ConsoleOut0.rtc.
```
 $ rtdis /localhost/ConsoleIn0.rtc:out /localhost/ConsoleOut0.rtc:in
```

- Disconnects all connections connected to the out port of ConsoleIn0.rtc.
```
 $ rtdis /localhost/ConsoleIn0.rtc:out
```

- Disconnects all connections from all ports of ConsoleIn0.rtc.
```
 $ rtdis /localhost/ConsoleIn0.rtc
```

- Disconnects only the connection with the ID con1 from the out port of ConsoleIn0.rtc.
```
 $ rtdis -i con1 /localhost/ConsoleIn0.rtc:out
```

- Disconnects only the connection with the ID con1 among the ports of ConsoleIn0.rtc.
```
