...
## Overview
Checks for inconsistencies between the RTSProfile of the running RT system and the file RTSPROFILE_FILE. It reports errors such as components in incorrect states and incorrect connections.

By default, the RTSProfile file is treated as XML format. To use YAML format, specify the --yaml (-y) option.

## Options (OPTION)
```
 --dry-run　　　Displays what will be done, but does not actually execute it.
 -s STATE, --state=STATE
 　　　　　　 Specifies the expected state of all components. The default is Active.
...
Some commands, such as rtiject, can create new ports. In this case, you can add them to the path with options. The available options are the name of the created port and the formatter. Specify them as follows:
```
...
## Examples
- Checks the RTSProfile file named sys.rtsys and verifies that the system is running without errors and that all components are in the active state.
```
 $ rtcheck sys.rtsys
```

- Same as above, except that all components are in the inactive state.
```
 $ rtcheck sys.rtsys -s Inactive
```

- Displays what will be done for the system check.
```
