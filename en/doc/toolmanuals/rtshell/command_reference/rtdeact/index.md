...
## Overview
Deactivates RT Components in the Active state.

```
Options
 -e EC_ID, --exec_context=EC_ID
 　　　　　　ID of the Execution context of the RTC to deactivate. The default is 0.
...
## Examples
- Deactivates ConsoleOut0.rtc on all ECs.
```
 $ rtdeact /localhost/local.host_cxt/ConsoleOut0.rtc
```

- Deactivates ConsoleOut0.rtc and ConsoleIn0.rtc on all ECs.
```
 $ rtdeact ConsoleOut0.rtc ConsoleIn0.rtc
```

- Deactivates Motor0.rtc only on the EC whose ID is "2".
```
