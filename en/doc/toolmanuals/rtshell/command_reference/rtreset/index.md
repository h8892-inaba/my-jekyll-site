...
## Overview
Resets RT Components that are in the Error state.

## Options (OPTION)
```
 -e EC_INDEX, --exec_context=EC_INDEX
 　　　　　　　Resets only within this execution context. The default is 0.
...
# Examples
- Resets ConsoleOut0.rtc on all ECs.
```
 $ rtreset /localhost/local.host_cxt/ConsoleOut0.rtc
```

- Resets ConsoleOut0.rtc and ConsoleIn0.rtc on all ECs.
```
...
- Resets Motor0.rtc only on the EC whose ID is "2".
```
